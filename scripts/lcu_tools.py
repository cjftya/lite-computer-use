...
def windows_backend() -> Any:
    if os.name != "nt":
        raise LCUError("unsupported_platform", "Lite Computer Use v1 requires Windows")
...
def run_action(args: argparse.Namespace) -> Any:
    ...
    dispatch: dict[str, Callable[[], Any]] = {
        ...
        "click": lambda: backend.click(args.x, args.y, args.relative_to, clicks=1),
        "double_click": lambda: backend.click(
            args.x, args.y, args.relative_to, clicks=2
        ),
        "scroll": lambda: backend.scroll(args.amount),
        "type_text": lambda: backend.type_text(args.text, args.interval),
        "press_key": lambda: backend.press_key(args.key),
...
def _screenshot(backend: Any, args: argparse.Namespace) -> Any:
    ...
    return backend.screenshot(args.output, args.active_window, args.all_screens)
